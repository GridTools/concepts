...
4. within a computation, it is illegal to write to an external (non-temporary) field (or aliases pointing to the same memory location) if it was read read with horizontal offset in a preceding statement; this rule does not apply to temporaries,
